...
"""
    ...
    Fields are (if exist)
    Ref, value, Part, footprint, Datasheet, Manufacturer, MPN
...
writerow( out, ['Ref', 'Value', 'Manufacturer', 'MPN', 'Footprint', 'Datasheet' ] )

# Output all of the component information (One component per row)
for c in components:
    writerow( out, [c.getRef(), c.getValue(),
        c.getField("Manufacturer"), c.getField("MPN"),
        c.getFootprint(), c.getDatasheet()])
